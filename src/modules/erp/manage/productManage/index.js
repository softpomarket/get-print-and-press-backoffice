...
const columns = [
...

  {
    ...
    width: "10%",
    render: (text, record) => {
      return <>{record.isActive ? <label>เปิดใช้งาน</label> : <label>ปิดใช้งาน</label>}</>;
    },
  },
...
export default function ProductManage(props) {
  ...
  console.log("propertyGal", propertyGallery);
  console.log("propertyGalDisplay", promotionGalleryDisplay);
  ...
  const [imageProductDetailsGalleryURL, setImageProductDetailGalleryURL] = useState({
    loading: false,
    imageUrl: null,
    imagePath: null,
  });
  const [productDetailGalleryDisplay, setProductDetailGalleryDisplay] = useState([]);
  const [loadingProductDetailGallery, setLoadingProductDetailGallery] = useState(false);

  const [imageProductExampleGalleryURL, setImageProductExampleGalleryURL] = useState({
    loading: false,
    imageUrl: null,
    imagePath: null,
  });
  const [productExampleGalleryDisplay, setProductExampleGalleryDisplay] = useState([]);
  const [loadingProductExampleGallery, setLoadingProductExampleGallery] = useState(false);
  ...
  const optionProductDetailGalleryImage = {
    ...
    async onChange(info) {
      ...
      console.log("A1 --- : ", result?.results);
      if (info.file.status === "done") {
        setLoadingProductDetailGallery(true);
        ...
        setLoadingProductDetailGallery(false);
      } else if (info.file.status === "error") {
        Notification("error", "เเจ้งเตือน!", "อัพโหลดรูปภาพไม่สำเร็จ กรุณาลองใหม่อีกครั้ง");
      }
    },
  ...
  const optionProductExampleGalleryImage = {
    ...
    async onChange(info) {
      ...
      console.log("A1 --- : ", result?.results);
      if (info.file.status === "done") {
        setLoadingProductExampleGallery(true);
        ...
        setLoadingProductExampleGallery(false);
      } else if (info.file.status === "error") {
        Notification("error", "เเจ้งเตือน!", "อัพโหลดรูปภาพไม่สำเร็จ กรุณาลองใหม่อีกครั้ง");
      }
    },
  ...
  const getPickUpAlls = async (name) => {
    ...
    const result = await getPickUpFetch(param, null, accessToken);
            ...
            <Button
              ...
              onClick={async () => {
                formProperty.setFieldsValue({
                  ...
                  price: val.price,
                  fileId: val.PickUpCover.id,
                  isActive: val.isActive,
                });

                setImageCoverProduct({
                  loading: false,
                  imageUrl: val.PickUpCover.googleImage,
                  imagePath: val.PickUpCover.imagePath,
                });
                //myEdit
                setPropertyGallery(val.PickUpGallery);
                setPromotionGalleryDisplay(val.PickUpGallery);
  ...
  const onPropertyFinish = async (values) => {
    ...
    const listOfPromotionGallery = promotionGalleryDisplay?.map((gal) => gal.imagePath);
    const listOfDetailGallery = productDetailGalleryDisplay?.map((gal) => gal.imagePath);
    const listOfExampleGallery = productExampleGalleryDisplay?.map((gal) => gal.imagePath);
    ...
    //myEdit
    let listOfURLById;
    if (promotionGalleryDisplay?.length === 0) {
      listOfURLById = [{ id: null, imagePath: null }];
    } else {
      listOfURLById = promotionGalleryDisplay?.map((property) => {
    ...
    console.log("checkiung", listOfURLById);
    ...
    if (modalProperty.title === "add") {
      const result = await insertPickUpFetch(null, body, accessToken);
      ...
      const result = await updatePickUpFetch(param, bodyUpdate, accessToken);
  ...
  const handlePropertiesDelete = async (id) => {
    ...
    const result = await deletePickUpByIdFetch(param, null, accessToken);
    if (result.status) {
  ...
  const handlePropertiesGalleryDelete = async (id) => {
    ...
    let tmpPropertyGalleryDisplay = promotionGalleryDisplay?.filter((fill) => fill.id !== id);

    console.log("tmpPropertyGallery", tmpPropertyGallery);
    //myEdit

    setPromotionGalleryDisplay(tmpPropertyGalleryDisplay);
  ...
  console.log("promotionGalleryDisplay", promotionGalleryDisplay);
        ...
        title={
          <strong>
            <label className="topic-color-bold">
              {modalProperty.title === "add" ? "สร้าง Product" : "เเก้ไข Product"}
            </label>
          </strong>
        }
          ...
          <Row gutter={[24, 0]}>
            <Col xs={24}>
              <Row gutter={[24, 0]}>
                    ...
                    <div style={{ paddingTop: 24, paddingBottom: 24 }}>
                      ...
                      >
                        <Button
                          ...
                          style={{ width: "100%" }}
                          icon={
                            imageCoverProduct.loading ? <LoadingOutlined /> : <UploadOutlined />
                          }
                        >
                          Upload ภาพ
                        </Button>
                ...
                <Col xs={24} md={12}>
                  <div style={{ display: "grid" }}>
                    <label style={{ paddingBottom: 6 }}>ภาพปก โฆษณาของ Product</label>
                    ...
                    <div style={{ paddingTop: 24, paddingBottom: 24 }}>
                      ...
                      >
                        <Button
                          type="primary"
                          style={{ width: "100%" }}
                          icon={imageAdsProduct.loading ? <LoadingOutlined /> : <UploadOutlined />}
                        >
                          Upload ภาพ
                        </Button>
                      </Upload>
                ...
                <Col xs={24} md={12}>
                  <div style={{ display: "grid" }}>
                    <label style={{ paddingBottom: 6 }}>ภาพ สินค้าแนะนำใกล้เคียง</label>
                    ...
                    <div style={{ paddingTop: 24, paddingBottom: 24 }}>
                      ...
                      >
                        <Button
                          ...
                          style={{ width: "100%" }}
                          icon={
                            imageProductRecommend.loading ? <LoadingOutlined /> : <UploadOutlined />
                          }
                        >
                          Upload ภาพ
                        </Button>
                  ...
                  >
                    ...
                    >
                      <Button
                        ...
                        style={{ width: "100%" }}
                        icon={
                          imageProductGalleryURL.loading ? <LoadingOutlined /> : <UploadOutlined />
                        }
                      >
                        เพิ่มภาพโปรโมชั่น
                      </Button>
