...
export default function AddOnManage(props) {
  ...
  const getPickUpAlls = async (name) => {
    ...
    const result = await getAddOnFetch(param, null, accessToken);
            ...
            <Button
              ...
              onClick={async () => {
                formProperty.setFieldsValue({
                  ...
                  addOnCategoryName: val.AddOnCategory.name,
                  addOnCategoryId: val.AddOnCategory.id,
                  price: val.price,
                  fileId: val.AddOnCover.id,
  ...
  const onPropertyFinish = async (values) => {
    ...
    if (modalProperty.title === "add") {
      const result = await insertAddOnFetch(null, body, accessToken);
      ...
      const result = await updateAddOnFetch(param, bodyUpdate, accessToken);
  ...
  const handlePropertiesDelete = async (id) => {
    ...
    const result = await deleteAddOnByIdFetch(param, null, accessToken);
    if (result.status) {
  ...
  const getAllCategories = async () => {
    try {
      const res = await fetch(`${serverUrl}/api/v1/add-on/category`);
  ...
  return (
    <Row>
      <Col span={12}>
        <label>ส่วนเสริมรถบรรทุก ISUZU</label>
      </Col>
          ...
          >
            <div style={{ paddingLeft: 10 }}>
              <Form.Item
                ...
                style={{ width: "100%" }}
              >
                <Input placeholder="ชื่อส่วนเสริมรถบรรทุก ISUZU" />
              </Form.Item>
            </div>
            ...
            <Col xs={24} md={12} xl={12}>
              <Row gutter={[24, 0]}>
                ...
                <Col span={24}>
                  ...
                  <Form.Item
                    name="model"
                    label="ชื่อส่วนเสริมรถบรรทุก ISUZU"
                    rules={[
                      {
                        required: true,
                ...
                <Col span={24}>
                  <Form.Item
                    ...
                    label="ประเภทส่วนเสริม"
                    rules={[
                      { required: true, message: "กรุณาเลือกประเภทส่วนเสริม" },
                    ]}
                  >
                    <Select
                      showSearch
